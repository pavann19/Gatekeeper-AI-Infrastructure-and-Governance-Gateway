...
- [x] **Full MCP Transport Support (stdio + HTTP/SSE)**: `core/mcp_server.py` (stdio) and `core/mcp_http_server.py` (networked HTTP/SSE with per-request API key auth, tenant isolation, session-relay queue, and 100KB payload bounding). The first implementation pass's SSE session relay was dead code (queue created, never written to; any `sessionId` including an unopened one was accepted); a follow-up commit wired the real relay through. A subsequent independent review found and fixed 3 more real issues before merge: the session relay wasn't actually reachable from a real SSE stream read (only unit-tested against a manually-injected queue) — verified live end-to-end against a real running server; the rate limiter reused the main API's `assess_rate_limiter` singleton instead of a dedicated one, a real cross-traffic-class budget-sharing bug once Redis-backed; and parse/size-limit errors returned FastAPI's generic error shape instead of a JSON-RPC 2.0 envelope. All 3 fixed and verified live, `docs/THREAT_MODEL.md` §7 findings #13-15.

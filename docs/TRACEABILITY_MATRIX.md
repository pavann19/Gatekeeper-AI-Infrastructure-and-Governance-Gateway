...
| The output toxicity judge must not silently fail open when the primary backend (Ollama/Llama Guard) is unreachable | `core/semantic_judge.py::_fallback_output_judge` (local `toxic_bert` fallback engaged before `JUDGE_OFFLINE` is ever returned), `OUTPUT_JUDGE_FALLBACK_ENABLED` | `tests/test_semantic_judge.py` (7 tests: both dispatch paths' outage routing, real-detector scoring shape, disabled-by-config, fallback-itself-unavailable), `tests/test_semantic_judge_edge_cases.py::test_output_judge_timeout_falls_back_to_inverter_not_bare_judge_offline` | ✅ (real gap found and closed this session; verified live against the real model — 71ms warm-call latency, correct classification) |
